...
def init_concat_wf(
    ...
    select_confoundsmeta = pe.Node(niu.Select(index=0), name="select_confoundsmeta")
    
    # Report number of runs included in new bold run
    report_concat = pe.Node(
        SessionSummary(),
        name="concat_summary"
    )
...
def _concat_bold(in_files, croprun):
    ...
    if croprun != 0:
        ...
        for f in in_files:
            nii = nib.load(f)
            croped_file = nib.Nifti1Image(nii.get_fdata()[:,:,:,croprun:], nii.affine)
...
def _concat_confounds(in_files, croprun):
    ...
    if len(in_files) > 1:
        concat = concat.drop([c for c in concat.columns if "comp" in c], axis=1)
